 ...
 * @returns {(numEntries: number, valueGetter?: (propName: string, index: number) => any) => any[]} resulting factory function
 */
function createObjectListFactoryFromSchema (schema, { deleteUndefined = true } = {}) {
	 ...
	 * @param {(propName: string, index: number) => any} valueGetter function used to get the value for a given property name and index
	 */
	return function (numEntries, valueGetter) {
		const valueGetterIsFunction = typeof valueGetter === 'function';
		const getValueOrDefault = (propName, index, defaultValue) => valueGetterIsFunction ? valueGetter(propName, index) : defaultValue;
