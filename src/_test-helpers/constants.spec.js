const effectMetadata = require('../buffs/effect-metadata');
...
describe('test setup', () => {
	...
	it('has a valid attacking proc id for KNOWN_ARBITRARY_ATTACKING_PROC_ID', () => {
		const metadataEntry = effectMetadata.PROC_METADATA[testConstants.KNOWN_ARBITRARY_ATTACKING_PROC_ID];
		...
		expect(metadataEntry.Type)
			...
			.toBeTruthy();
		expect(metadataEntry.Type)
			.withContext(`Test Proc ID ${testConstants.KNOWN_ARBITRARY_ATTACKING_PROC_ID} is not type ${effectMetadata.ProcBuffType.Attack}`)
			.toBe(effectMetadata.ProcBuffType.Attack);
	});

	it('has a valid non-attacking proc id for KNOWN_ARBITRARY_NON_ATTACKING_PROC_ID', () => {
		const metadataEntry = effectMetadata.PROC_METADATA[testConstants.KNOWN_ARBITRARY_NON_ATTACKING_PROC_ID];
		expect(metadataEntry)
		...
		expect(metadataEntry.Type)
			...
			.toBeDefined();
		expect(metadataEntry.Type)
			.withContext(`Test Proc ID ${testConstants.KNOWN_ARBITRARY_NON_ATTACKING_PROC_ID} should not be type ${effectMetadata.ProcBuffType.Attack}`)
			.not.toBe(effectMetadata.ProcBuffType.Attack);
	});

	it('has an invalid proc id for ARBITRARY_INVALID_PROC_ID', () => {
		const metadataEntry = effectMetadata.PROC_METADATA[testConstants.ARBITRARY_INVALID_PROC_ID];
		expect(metadataEntry)
	...
	it('has a valid passive id for KNOWN_ARBITRARY_PASSIVE_ID', () => {
		const metadataEntry = effectMetadata.PASSIVE_METADATA[testConstants.KNOWN_ARBITRARY_PASSIVE_ID];
		expect(metadataEntry)
	...
	it('has an invalid passive id for ARBITRARY_INVALID_PASSIVE_ID', () => {
		const metadataEntry = effectMetadata.PROC_METADATA[testConstants.ARBITRARY_INVALID_PASSIVE_ID];
		expect(metadataEntry)
