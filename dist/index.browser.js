...
var bfmtUtilities = function (exports) {
  ...
  var constants = /*#__PURE__*/Object.freeze({
    ...
    get KNOWN_CONDITIONAL_ID() {
      return KNOWN_CONDITIONAL_ID;
    }
  ...
  (function (IconId) {
    ...
    IconId["BUFF_DAMAGECUTTOONE"] = "BUFF_DAMAGECUTTOONE"; // elemental damage reduction buffs
    ...
    IconId["BUFF_ELEMENTDMGDOWN"] = "BUFF_ELEMENTDMGDOWN"; // elemental weakness buffs
  ...
  function setMapping(map) {
    ...
    map.set('48', (effect, context, injectionContext) => {
      ...
      const createAttackOfType = (type, valueProperties) => Object.assign({
        id: `proc:48:piercing attack-${type}`,
        originalId,
        sources,
        effectDelay,
        value: Object.assign(Object.assign({}, valueProperties), {
          isLethal,
          chance,
          hits,
          distribution
    ...
    map.set('51', (effect, context, injectionContext) => {
      ...
      if (effect.params) {
        const params = splitEffectParams(effect);
        [{
          type: Ailment.AttackReduction,
          reductionValue: parseNumberOrDefault(params[0]),
          chance: parseNumberOrDefault(params[3])
        }, {
          type: Ailment.DefenseReduction,
          reductionValue: parseNumberOrDefault(params[1]),
          chance: parseNumberOrDefault(params[4])
        }, {
          type: Ailment.RecoveryReduction,
          reductionValue: parseNumberOrDefault(params[2]),
          chance: parseNumberOrDefault(params[5])
        }].forEach(({
          type,
          reductionValue,
          chance
        ...
        [{
          type: Ailment.AttackReduction,
          reductionValueKey: 'inflict atk% debuff (2)',
          chanceKey: 'inflict atk% debuff chance% (74)'
        }, {
          type: Ailment.DefenseReduction,
          reductionValueKey: 'inflict def% debuff (4)',
          chanceKey: 'inflict def% debuff chance% (75)'
        }, {
          type: Ailment.RecoveryReduction,
          reductionValueKey: 'inflict rec% debuff (6)',
          chanceKey: 'inflict rec% debuff chance% (76)'
        }].forEach(({
          type,
          reductionValueKey,
          chanceKey
      ...
      const results = inflictedReductions.map(({
        type,
        reductionValue,
        chance
      }) => Object.assign({
    ...
    map.set('55', (effect, context, injectionContext) => {
      ...
      if (effect.params) {
        const params = splitEffectParams(effect);
        elements = params.filter((value, index) => value !== '0' && index < 6).map(e => NON_ZERO_ELEMENT_MAPPING[e] || BuffConditionElement.Unknown);
      ...
      if (damageBoost !== 0) {
        results = elements.map(element => Object.assign({
    ...
    map.set('56', (effect, context, injectionContext) => {
      ...
      if (chance !== 0) {
        results.push(Object.assign({
          ...
          duration: turnDuration,
          value: {
            'recoveredHp%': recoveredHpPercent,
            chance
          }
        }, targetData));
      } else if (isTurnDurationBuff(context, turnDuration, injectionContext)) {
    ...
    map.set('57', (effect, context, injectionContext) => {
      ...
      if (results.length === 0 && isTurnDurationBuff(context, turnDuration, injectionContext)) {
        results.push(createTurnDurationEntry({
          originalId,
          sources,
          buffs: ['proc:57:bc drop resistance-base', 'proc:57:bc drop resistance-buff', 'proc:57:hc drop resistance-base', 'proc:57:hc drop resistance-buff'],
          duration: turnDuration,
          targetData
        }));
      }
  ...
  function convertConditionalEffectToBuffs(effect, context) {
    ...
    const conversionFunction = getConditionalEffectToBuffMapping(context.reloadMapping).get(effect.id); // TODO: warning if result is empty?
  ...
  function setMapping$2(map) {
    ...
    map.set('50', (effect, context, injectionContext) => {
      ...
      if (typedEffect.params) {
        const params = splitEffectParams(typedEffect);
        elements = params.filter((value, index) => value !== '0' && index < 6).map(e => ELEMENT_MAPPING[e] || BuffConditionElement.Unknown);
        damageBoost = parseNumberOrDefault(params[6]) * 100;
        unknownParams = createUnknownParamsEntryFromExtraParams(params.slice(7), 7, injectionContext);
      } else {
    ...
    map.set('53', (effect, context, injectionContext) => {
      ...
      if (typedEffect.params) {
        ...
        [{
          resistType: ResistType.CriticalDamage,
          base: parseNumberOrDefault(rawBaseCritDamageResist),
          buff: parseNumberOrDefault(rawBuffCritDamageResist)
        }, {
          resistType: ResistType.ElementDamage,
          base: parseNumberOrDefault(rawBaseElementDamageResist),
          buff: parseNumberOrDefault(rawBuffElementDamageResist)
        }, {
          resistType: ResistType.CriticalHitRate,
          base: parseNumberOrDefault(rawBaseCritChanceResist),
          buff: parseNumberOrDefault(rawBuffCritChanceResist)
        }].forEach(({
          resistType,
          base,
          buff
        ...
        [{
          resistType: ResistType.CriticalDamage,
          baseKey: 'crit dmg base damage resist%',
          buffKey: 'crit dmg buffed damage resist%'
        }, {
          resistType: ResistType.ElementDamage,
          baseKey: 'strong base element damage resist%',
          buffKey: 'strong buffed element damage resist%'
        }, {
          resistType: ResistType.CriticalHitRate,
          baseKey: 'crit chance base resist%',
          buffKey: 'crit chance buffed resist%'
        }].forEach(({
          resistType,
          baseKey,
          buffKey
  ...
  function convertPassiveEffectToBuffs(effect, context) {
    ...
    return typeof conversionFunction === 'function' ? conversionFunction(effect, context) : defaultConversionFunction$2(effect, context);
  }
